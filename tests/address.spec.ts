...
test.describe('Export', () => {
    ...
    test('Verify address data export matches API', async ({ page, browserName, request }) => {
        ...
        // Step 2: Filter row that has Address 1 value
        const rowWithAddress = data.find(row => {
            const address1 = row['Address 1']?.toString().trim();
            return address1 && address1 !== '';
        });

        if (!rowWithAddress) {
            console.log('⚠️ No rows with Address 1 found in export - skipping test');
            test.skip();
            return;
        }

        const vespistiId = rowWithAddress['Vespisti ID']?.toString();
        expect(vespistiId).toBeTruthy();
        console.log(`🔍 Testing Vespisti ID: ${vespistiId}`);
        console.log(`📍 Excel Address 1: ${rowWithAddress['Address 1']}`);

        // Step 3: Get internal ID from List API
        // Use page.request to call API with same auth context
        const listApiUrl = '/api/customer?page=1&limit=100&order_by=created_at&order_dir=desc';

        let internalId: string | null = null;
        let currentPage = 1;
        const maxPages = 10; // Limit pages to search

        while (!internalId && currentPage <= maxPages) {
            const listUrl = `/api/customer?page=${currentPage}&limit=100&order_by=created_at&order_dir=desc`;
            const listResponse = await page.request.get(listUrl);
            expect(listResponse.ok(), `List API should return OK, got ${listResponse.status()}`).toBe(true);

            const listData = await listResponse.json();
            const customers = listData.data || listData.items || listData;

            if (Array.isArray(customers)) {
                const found = customers.find((c: any) => c.vespisti_code === vespistiId);
                if (found) {
                    internalId = found.id;
                    console.log(`✅ Found internal ID: ${internalId} for ${vespistiId}`);
                    break;
                }
            }

            // Check if more pages
            const totalPages = listData.totalPages || listData.total_pages || Math.ceil((listData.total || 0) / 100);
            if (currentPage >= totalPages) break;
            currentPage++;
        }

        expect(internalId, `Could not find internal ID for ${vespistiId}`).toBeTruthy();

        // Step 4: Call Detail API to get addresses
        const detailApiUrl = `/api/customer/${internalId}`;
        const detailResponse = await page.request.get(detailApiUrl);
        expect(detailResponse.ok(), `Detail API should return OK, got ${detailResponse.status()}`).toBe(true);

        const detailData = await detailResponse.json();
        const customerData = detailData.data || detailData;
        const apiAddresses = customerData.addresses || [];

        console.log(`📦 API returned ${apiAddresses.length} addresses`);

        // Step 5: Compare address data
        expect(apiAddresses.length).toBeGreaterThan(0);

        for (let i = 0; i < apiAddresses.length; i++) {
            const idx = i + 1;  // Excel columns: 1, 2, 3, 4, 5
            const apiAddr = apiAddresses[i];

            console.log(`\n🔍 Comparing Address ${idx}:`);

            // Address: API field is "address1" (masked), Excel is full
            // Verify Excel starts with the unmasked prefix from API
            const apiAddressValue = apiAddr.address1?.toString() || '';
            const apiAddressPrefix = apiAddressValue.replace(/\*/g, ''); // "199*****" -> "199"
            const excelAddress = rowWithAddress[`Address ${idx}`]?.toString() || '';

            if (apiAddressPrefix) {
                expect(
                    excelAddress.startsWith(apiAddressPrefix),
                    `Excel Address ${idx} should start with "${apiAddressPrefix}", got: "${excelAddress}"`
                ).toBe(true);
                console.log(`  ✅ Address: Excel "${excelAddress.substring(0, 20)}..." starts with API prefix "${apiAddressPrefix}"`);
            }

            // Sub district: API field is "sub_district_name"
            const apiSubDistrict = apiAddr.sub_district_name || '';
            const excelSubDistrict = rowWithAddress[`Sub district ${idx}`]?.toString() || '';
            expect(
                excelSubDistrict,
                `Excel Sub district ${idx} should be "${apiSubDistrict}", got: "${excelSubDistrict}"`
            ).toBe(apiSubDistrict);
            console.log(`  ✅ Sub district: "${excelSubDistrict}"`);

            // District: API field is "district_name"
            const apiDistrict = apiAddr.district_name || '';
            const excelDistrict = rowWithAddress[`District ${idx}`]?.toString() || '';
            expect(
                excelDistrict,
                `Excel District ${idx} should be "${apiDistrict}", got: "${excelDistrict}"`
            ).toBe(apiDistrict);
            console.log(`  ✅ District: "${excelDistrict}"`);

            // Province: API field is "province_name"
            const apiProvince = apiAddr.province_name || '';
            const excelProvince = rowWithAddress[`Province ${idx}`]?.toString() || '';
            expect(
                excelProvince,
                `Excel Province ${idx} should be "${apiProvince}", got: "${excelProvince}"`
            ).toBe(apiProvince);
            console.log(`  ✅ Province: "${excelProvince}"`);

            // Postcode: API field is "postcode"
            const apiPostcode = String(apiAddr.postcode || '');
            const excelPostcode = String(rowWithAddress[`Postcode ${idx}`] || '');
            expect(
                excelPostcode,
                `Excel Postcode ${idx} should be "${apiPostcode}", got: "${excelPostcode}"`
            ).toBe(apiPostcode);
